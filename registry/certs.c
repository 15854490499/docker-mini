...
static char *corresponding_cert_name(const char *key_name)
{
    ...
    tmp_key_name = strdup_s(key_name);
...
static int get_path_by_cert_name(const char *path, const char *cert_name, char **cert_path, char **key_path)
{
    ...
    tmp_key_path = path_join(path, key_name);
    tmp_cert_path = path_join(path, cert_name);
    ...
    *cert_path = _strdup_s(tmp_cert_path);
    *key_path = strdup_s(tmp_key_path);
...
static int get_path_by_key_name(const char *path, const char *key_name, char **cert_path, char **key_path)
{
    ...
    *cert_path = strdup_s(tmp_cert_path);
    *key_path = strdup_s(tmp_key_path);
                      ...
                      char **key_file)
{
    ...
    if (has_suffix(name, CA_SUFFIX)) {
        if (*ca_file != NULL) {
            printf("more than one ca file found, support only one ca file currently, continue to try");
            goto out;
        }
        *ca_file = path_join(path, name);
        if (*ca_file == NULL) {
            ret = -1;
            printf("error join ca suffix");
            goto out;
        }
        goto out;
    } else if (has_suffix(name, CLIENT_CERT_SUFFIX)) {
        ret = get_path_by_cert_name(path, name, &tmp_cert_file, &tmp_key_file);
        if (ret != 0) {
            printf("get path of cert and key by cert name failed");
            //isulad_try_set_error_message("get path of cert and key by cert name failed");
            goto out;
        }
        if (!file_exists(tmp_key_file)) {
            ret = -1;
            printf("lack corresponding key file for tls cert");
            //isulad_try_set_error_message("lack corresponding key file for tls cert");
            goto out;
        }
        if (*cert_file != NULL) {
            printf("more than one cert file found, support only one cert file currently, continue to try");
            goto out;
        }
        *cert_file = strdup_s(tmp_cert_file);
        goto out;
    } else if (has_suffix(name, CLIENT_KEY_SUFFIX)) {
        ret = get_path_by_key_name(path, name, &tmp_cert_file, &tmp_key_file);
        if (ret != 0) {
            printf("get path of cert and key by key name failed");
            //isulad_try_set_error_message("get path of cert and key by key name failed");
            goto out;
        }
        if (!file_exists(tmp_cert_file)) {
            ret = -1;
            printf("lack corresponding cert file for tls key");
            //isulad_try_set_error_message("lack corresponding cert file for tls key");
            goto out;
        }
        if (*key_file != NULL) {
            printf("more than one key file found, support only one key file currently, continue to try");
            goto out;
        }
        *key_file = strdup_s(tmp_key_file);
        goto out;
    } else {
        goto out;
    }
...
int certs_load(char *host, bool use_decrypted_key, char **ca_file, char **cert_file, char **key_file)
{
    ...
    path = path_join(g_certs_dir, host);
    if (path == NULL) {
    ...
    if (!valid_certs(*ca_file, *cert_file, *key_file)) {
        printf("failed to load all certs");
