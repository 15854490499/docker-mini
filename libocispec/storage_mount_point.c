...
define_cleaner_function (storage_mount_point *, free_storage_mount_point)
storage_mount_point *
make_storage_mount_point (yajl_val tree, const struct parser_context *ctx, parser_error *err)
{
    __auto_cleanup(free_storage_mount_point) storage_mount_point *ret = NULL;
    *err = NULL;
        ...
        if (val != NULL)
          {
            ...
            invalid = common_safe_int32 (YAJL_GET_NUMBER (val), &ret->count);
            if (invalid)
              {
                if (asprintf (err, "Invalid value '%s' with type 'int32' for key 'count': %s", YAJL_GET_NUMBER (val), strerror (-invalid)) < 0)
    ...
    if (tree->type == yajl_t_object)
      {
        ...
        for (i = 0; i < tree->u.object.len; i++)
          {if (strcmp (tree->u.object.keys[i], "id")
                && strcmp (tree->u.object.keys[i], "count")
...
void
free_storage_mount_point (storage_mount_point *ptr)
{
    if (ptr == NULL)
        return;
    free (ptr->id);
...
yajl_gen_status
gen_storage_mount_point (yajl_gen g, const storage_mount_point *ptr, const struct parser_context *ctx, parser_error *err)
{
...
storage_mount_point *
storage_mount_point_parse_file (const char *filename, const struct parser_context *ctx, parser_error *err)
{
storage_mount_point *ptr = NULL;size_t filesize;
    ...
    if (content == NULL)
      {
        if (asprintf (err, "cannot read the file: %s", filename) < 0)
            *err = strdup ("error allocating memory");
        return NULL;
      }ptr = storage_mount_point_parse_data (content, ctx, err);return ptr;
}
storage_mount_point * 
storage_mount_point_parse_file_stream (FILE *stream, const struct parser_context *ctx, parser_error *err)
{storage_mount_point *ptr = NULL;
size_t filesize;
    __auto_free char *content = NULL;
...
define_cleaner_function (yajl_val, yajl_tree_free)
 ...
 { 
  storage_mount_point *ptr = NULL;__auto_cleanup(yajl_tree_free) yajl_val tree = NULL;
    char errbuf[1024];
...
ptr = make_storage_mount_point (tree, ctx, err);return ptr; 
}
...
define_cleaner_function (yajl_gen, cleanup_yajl_gen)
 ...
 char * 
storage_mount_point_generate_json (const storage_mount_point *ptr, const struct parser_context *ctx, parser_error *err){
...
if (yajl_gen_status_ok != gen_storage_mount_point (g, ptr, ctx, err))  {
        if (*err == NULL)
