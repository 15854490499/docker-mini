...
define_cleaner_function (oci_image_manifest *, free_oci_image_manifest)
oci_image_manifest *
make_oci_image_manifest (yajl_val tree, const struct parser_context *ctx, parser_error *err)
{
    __auto_cleanup(free_oci_image_manifest) oci_image_manifest *ret = NULL;
    *err = NULL;
    ...
    while (0);
    ret->config = make_oci_image_content_descriptor (get_val (tree, "config", yajl_t_object), ctx, err);
            ...
            for (i = 0; i < len; i++)
              {
                ...
                ret->layers[i] = make_oci_image_content_descriptor (val, ctx, err);
...
void
free_oci_image_manifest (oci_image_manifest *ptr)
{
    if (ptr == NULL)
        return;
    if (ptr->config != NULL)
      {
        free_oci_image_content_descriptor (ptr->config);
        ptr->config = NULL;
      }
    if (ptr->layers != NULL)      {
        size_t i;
        for (i = 0; i < ptr->layers_len; i++)
          ...
          if (ptr->layers[i] != NULL)
            {
              free_oci_image_content_descriptor (ptr->layers[i]);
              ptr->layers[i] = NULL;
            }
          }
        free (ptr->layers);
...
yajl_gen_status
gen_oci_image_manifest (yajl_gen g, const oci_image_manifest *ptr, const struct parser_context *ctx, parser_error *err)
{
    ...
    if ((ctx->options & OPT_GEN_KEY_VALUE) || (ptr != NULL && ptr->config != NULL))
      {
        ...
        if (stat != yajl_gen_status_ok)
            GEN_SET_ERROR_AND_RETURN (stat, err);
        stat = gen_oci_image_content_descriptor (g, ptr != NULL ? ptr->config : NULL, ctx, err);
        if (stat != yajl_gen_status_ok)
            GEN_SET_ERROR_AND_RETURN (stat, err);
      }
    if ((ctx->options & OPT_GEN_KEY_VALUE) || (ptr != NULL && ptr->layers != NULL))
      {
        ...
        for (i = 0; i < len; i++)
          {
            stat = gen_oci_image_content_descriptor (g, ptr->layers[i], ctx, err);
            if (stat != yajl_gen_status_ok)
                GEN_SET_ERROR_AND_RETURN (stat, err);
          }
...
oci_image_manifest *
oci_image_manifest_parse_file (const char *filename, const struct parser_context *ctx, parser_error *err)
{
oci_image_manifest *ptr = NULL;size_t filesize;
    ...
    *err = NULL;
	...
	ptr = oci_image_manifest_parse_data (content, ctx, err);return ptr;
}
oci_image_manifest * 
oci_image_manifest_parse_file_stream (FILE *stream, const struct parser_context *ctx, parser_error *err)
{oci_image_manifest *ptr = NULL;
size_t filesize;
    __auto_free char *content = NULL;
...
define_cleaner_function (yajl_val, yajl_tree_free)
 ...
 {
  oci_image_manifest *ptr = NULL;__auto_cleanup(yajl_tree_free) yajl_val tree = NULL;
    char errbuf[1024];
...
ptr = make_oci_image_manifest (tree, ctx, err);return ptr; 
}
...
define_cleaner_function (yajl_gen, cleanup_yajl_gen)
 ...
 char * 
oci_image_manifest_generate_json (const oci_image_manifest *ptr, const struct parser_context *ctx, parser_error *err){
...
if (yajl_gen_status_ok != gen_oci_image_manifest (g, ptr, ctx, err))  {
        if (*err == NULL)
