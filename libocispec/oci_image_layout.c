...
#include <string.h>
#include "read-file.h"
#include "oci_image_layout.h"

#define YAJL_GET_ARRAY_NO_CHECK(v) (&(v)->u.array)
#define YAJL_GET_OBJECT_NO_CHECK(v) (&(v)->u.object)
define_cleaner_function (oci_image_layout *, free_oci_image_layout)
oci_image_layout *
make_oci_image_layout (yajl_val tree, const struct parser_context *ctx, parser_error *err)
{
    __auto_cleanup(free_oci_image_layout) oci_image_layout *ret = NULL;
    *err = NULL;
...
void
free_oci_image_layout (oci_image_layout *ptr)
{
    if (ptr == NULL)
        return;
...
yajl_gen_status
gen_oci_image_layout (yajl_gen g, const oci_image_layout *ptr, const struct parser_context *ctx, parser_error *err)
{
...
oci_image_layout *
oci_image_layout_parse_file (const char *filename, const struct parser_context *ctx, parser_error *err)
{
oci_image_layout *ptr = NULL;size_t filesize;
    ...
    if (content == NULL)
      {
        if (asprintf (err, "cannot read the file: %s", filename) < 0)
            *err = strdup ("error allocating memory");
        return NULL;
      }ptr = oci_image_layout_parse_data (content, ctx, err);return ptr;
}
oci_image_layout * 
oci_image_layout_parse_file_stream (FILE *stream, const struct parser_context *ctx, parser_error *err)
{oci_image_layout *ptr = NULL;
size_t filesize;
    __auto_free char *content = NULL;
...
define_cleaner_function (yajl_val, yajl_tree_free)
 ...
 { 
  oci_image_layout *ptr = NULL;__auto_cleanup(yajl_tree_free) yajl_val tree = NULL;
    char errbuf[1024];
...
ptr = make_oci_image_layout (tree, ctx, err);return ptr; 
}
...
define_cleaner_function (yajl_gen, cleanup_yajl_gen)
 ...
 char * 
oci_image_layout_generate_json (const oci_image_layout *ptr, const struct parser_context *ctx, parser_error *err){
...
if (yajl_gen_status_ok != gen_oci_image_layout (g, ptr, ctx, err))  {
        if (*err == NULL)
