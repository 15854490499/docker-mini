...
define_cleaner_function (storage_spec_config *, free_storage_spec_config)
storage_spec_config *
make_storage_spec_config (yajl_val tree, const struct parser_context *ctx, parser_error *err)
{
    __auto_cleanup(free_storage_spec_config) storage_spec_config *ret = NULL;
    *err = NULL;
...
void
free_storage_spec_config (storage_spec_config *ptr)
{
    if (ptr == NULL)
        return;
    free (ptr->user);
...
yajl_gen_status
gen_storage_spec_config (yajl_gen g, const storage_spec_config *ptr, const struct parser_context *ctx, parser_error *err)
{
...
define_cleaner_function (storage_spec_rootfs *, free_storage_spec_rootfs)
storage_spec_rootfs *
make_storage_spec_rootfs (yajl_val tree, const struct parser_context *ctx, parser_error *err)
{
    __auto_cleanup(free_storage_spec_rootfs) storage_spec_rootfs *ret = NULL;
    *err = NULL;
    (void) ctx;  /* Silence compiler warning.  */
    if (tree == NULL)
      return NULL;
...
void
free_storage_spec_rootfs (storage_spec_rootfs *ptr)
{
    if (ptr == NULL)
        return;
...
yajl_gen_status
gen_storage_spec_rootfs (yajl_gen g, const storage_spec_rootfs *ptr, const struct parser_context *ctx, parser_error *err)
{
    yajl_gen_status stat = yajl_gen_status_ok;
    *err = NULL;
    (void) ptr;  /* Silence compiler warning.  */
...
define_cleaner_function (storage_spec_history_element *, free_storage_spec_history_element)
storage_spec_history_element *
make_storage_spec_history_element (yajl_val tree, const struct parser_context *ctx, parser_error *err)
{
    __auto_cleanup(free_storage_spec_history_element) storage_spec_history_element *ret = NULL;
    *err = NULL;
    (void) ctx;  /* Silence compiler warning.  */
    if (tree == NULL)
      return NULL;
...
void
free_storage_spec_history_element (storage_spec_history_element *ptr)
{
    if (ptr == NULL)
        return;
    free (ptr->created);
...
yajl_gen_status
gen_storage_spec_history_element (yajl_gen g, const storage_spec_history_element *ptr, const struct parser_context *ctx, parser_error *err)
{
    yajl_gen_status stat = yajl_gen_status_ok;
    *err = NULL;
    (void) ptr;  /* Silence compiler warning.  */
...
define_cleaner_function (storage_spec *, free_storage_spec)
storage_spec *
make_storage_spec (yajl_val tree, const struct parser_context *ctx, parser_error *err)
{
    __auto_cleanup(free_storage_spec) storage_spec *ret = NULL;
    *err = NULL;
    (void) ctx;  /* Silence compiler warning.  */
    if (tree == NULL)
      return NULL;
    ...
    while (0);
    ret->config = make_storage_spec_config (get_val (tree, "config", yajl_t_object), ctx, err);
    if (ret->config == NULL && *err != 0)
      return NULL;
    ret->rootfs = make_storage_spec_rootfs (get_val (tree, "rootfs", yajl_t_object), ctx, err);
        ...
        if (tmp != NULL && YAJL_GET_ARRAY (tmp) != NULL)
          {
            ...
            for (i = 0; i < len; i++)
              {
                yajl_val val = values[i];
                ret->history[i] = make_storage_spec_history_element (val, ctx, err);
...
void
free_storage_spec (storage_spec *ptr)
{
    if (ptr == NULL)
        return;
    free (ptr->created);
    ...
    if (ptr->config != NULL)
      {
        free_storage_spec_config (ptr->config);
        ptr->config = NULL;
      }
    if (ptr->rootfs != NULL)
      {
        free_storage_spec_rootfs (ptr->rootfs);
        ptr->rootfs = NULL;
      }
    if (ptr->history != NULL)      {
        size_t i;
        for (i = 0; i < ptr->history_len; i++)
          ...
          if (ptr->history[i] != NULL)
            {
              free_storage_spec_history_element (ptr->history[i]);
              ptr->history[i] = NULL;
            }
          }
        free (ptr->history);
...
yajl_gen_status
gen_storage_spec (yajl_gen g, const storage_spec *ptr, const struct parser_context *ctx, parser_error *err)
{
    yajl_gen_status stat = yajl_gen_status_ok;
    *err = NULL;
    (void) ptr;  /* Silence compiler warning.  */
    ...
    if ((ctx->options & OPT_GEN_KEY_VALUE) || (ptr != NULL && ptr->config != NULL))
      {
        ...
        if (stat != yajl_gen_status_ok)
            GEN_SET_ERROR_AND_RETURN (stat, err);
        stat = gen_storage_spec_config (g, ptr != NULL ? ptr->config : NULL, ctx, err);
        if (stat != yajl_gen_status_ok)
            GEN_SET_ERROR_AND_RETURN (stat, err);
      }
    if ((ctx->options & OPT_GEN_KEY_VALUE) || (ptr != NULL && ptr->rootfs != NULL))
      {
        ...
        if (stat != yajl_gen_status_ok)
            GEN_SET_ERROR_AND_RETURN (stat, err);
        stat = gen_storage_spec_rootfs (g, ptr != NULL ? ptr->rootfs : NULL, ctx, err);
        if (stat != yajl_gen_status_ok)
            GEN_SET_ERROR_AND_RETURN (stat, err);
      }
    if ((ctx->options & OPT_GEN_KEY_VALUE) || (ptr != NULL && ptr->history != NULL))
      {
        ...
        for (i = 0; i < len; i++)
          {
            stat = gen_storage_spec_history_element (g, ptr->history[i], ctx, err);
            if (stat != yajl_gen_status_ok)
                GEN_SET_ERROR_AND_RETURN (stat, err);
          }
        stat = yajl_gen_array_close ((yajl_gen) g);
...
storage_spec *
storage_spec_parse_file (const char *filename, const struct parser_context *ctx, parser_error *err)
{
storage_spec *ptr = NULL;size_t filesize;
    ...
    if (content == NULL)
      {
        if (asprintf (err, "cannot read the file: %s", filename) < 0)
            *err = strdup ("error allocating memory");
        return NULL;
      }ptr = storage_spec_parse_data (content, ctx, err);return ptr;
}
storage_spec * 
storage_spec_parse_file_stream (FILE *stream, const struct parser_context *ctx, parser_error *err)
{storage_spec *ptr = NULL;
size_t filesize;
    __auto_free char *content = NULL;
...
define_cleaner_function (yajl_val, yajl_tree_free)
 ...
 { 
  storage_spec *ptr = NULL;__auto_cleanup(yajl_tree_free) yajl_val tree = NULL;
    char errbuf[1024];
...
ptr = make_storage_spec (tree, ctx, err);return ptr; 
}
...
define_cleaner_function (yajl_gen, cleanup_yajl_gen)
 ...
 char * 
storage_spec_generate_json (const storage_spec *ptr, const struct parser_context *ctx, parser_error *err){
...
if (yajl_gen_status_ok != gen_storage_spec (g, ptr, ctx, err))  {
        if (*err == NULL)
