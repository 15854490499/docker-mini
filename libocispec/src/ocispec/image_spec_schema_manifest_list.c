...
#include "image_spec_schema_manifest_list.h"

#define YAJL_GET_ARRAY_NO_CHECK(v) (&(v)->u.array)
#define YAJL_GET_OBJECT_NO_CHECK(v) (&(v)->u.object)
define_cleaner_function (image_spec_schema_manifest_list_manifests_platform *, free_image_spec_schema_manifest_list_manifests_platform)
image_spec_schema_manifest_list_manifests_platform *
make_image_spec_schema_manifest_list_manifests_platform (yajl_val tree, const struct parser_context *ctx, parser_error *err)
{
    __auto_cleanup(free_image_spec_schema_manifest_list_manifests_platform) image_spec_schema_manifest_list_manifests_platform *ret = NULL;
    *err = NULL;
    ...
    do
      {
        yajl_val tmp = get_val (tree, "features", yajl_t_array);
        if (tmp != NULL && YAJL_GET_ARRAY (tmp) != NULL)
          {
            size_t i;
            size_t len = YAJL_GET_ARRAY_NO_CHECK (tmp)->len;
            yajl_val *values = YAJL_GET_ARRAY_NO_CHECK (tmp)->values;
            ret->features_len = len;
            ret->features = calloc (len + 1, sizeof (*ret->features));
            ...
            for (i = 0; i < len; i++)
              {
                ...
                if (val != NULL)
                  {
                    char *str = YAJL_GET_STRING (val);
                    ret->features[i] = strdup (str ? str : "");
                    if (ret->features[i] == NULL)
                      return NULL;
                  }
              }
        }
    ...
    if (tree->type == yajl_t_object)
      {
        ...
        for (i = 0; i < tree->u.object.len; i++)
          {if (strcmp (tree->u.object.keys[i], "architecture")
                ...
                && strcmp (tree->u.object.keys[i], "features")){
                if (ctx->options & OPT_PARSE_FULLKEY)
                  {
...
void
free_image_spec_schema_manifest_list_manifests_platform (image_spec_schema_manifest_list_manifests_platform *ptr)
{
    if (ptr == NULL)
        return;
    ...
    if (ptr->features != NULL)
      {
        size_t i;
        for (i = 0; i < ptr->features_len; i++)
          {
            if (ptr->features[i] != NULL)
              {
                free (ptr->features[i]);
                ptr->features[i] = NULL;
              }
...
yajl_gen_status
gen_image_spec_schema_manifest_list_manifests_platform (yajl_gen g, const image_spec_schema_manifest_list_manifests_platform *ptr, const struct parser_context *ctx, parser_error *err)
{
    ...
    *err = NULL;
    ...
    if ((ctx->options & OPT_GEN_KEY_VALUE) || (ptr != NULL && ptr->features != NULL))
      {
        size_t len = 0, i;
        stat = yajl_gen_string ((yajl_gen) g, (const unsigned char *)("features"), 8 /* strlen ("features") */);
        if (stat != yajl_gen_status_ok)
            GEN_SET_ERROR_AND_RETURN (stat, err);
        if (ptr != NULL && ptr->features != NULL)
          len = ptr->features_len;
        if (!len && !(ctx->options & OPT_GEN_SIMPLIFY))
            yajl_gen_config (g, yajl_gen_beautify, 0);
        stat = yajl_gen_array_open ((yajl_gen) g);
        if (stat != yajl_gen_status_ok)
        ...
        for (i = 0; i < len; i++)
          {
            stat = yajl_gen_string ((yajl_gen)g, (const unsigned char *)(ptr->features[i]), strlen (ptr->features[i]));
            if (stat != yajl_gen_status_ok)
                GEN_SET_ERROR_AND_RETURN (stat, err);
          }
        stat = yajl_gen_array_close ((yajl_gen) g);
...
define_cleaner_function (image_spec_schema_manifest_list_manifests_element *, free_image_spec_schema_manifest_list_manifests_element)
image_spec_schema_manifest_list_manifests_element *
make_image_spec_schema_manifest_list_manifests_element (yajl_val tree, const struct parser_context *ctx, parser_error *err)
{
    __auto_cleanup(free_image_spec_schema_manifest_list_manifests_element) image_spec_schema_manifest_list_manifests_element *ret = NULL;
    *err = NULL;
    (void) ctx;  /* Silence compiler warning.  */
    if (tree == NULL)
      return NULL;
    ...
    while (0);
    ret->platform = make_image_spec_schema_manifest_list_manifests_platform (get_val (tree, "platform", yajl_t_object), ctx, err);
...
void
free_image_spec_schema_manifest_list_manifests_element (image_spec_schema_manifest_list_manifests_element *ptr)
{
    if (ptr == NULL)
        return;
    free (ptr->media_type);
    ...
    if (ptr->platform != NULL)
      {
        free_image_spec_schema_manifest_list_manifests_platform (ptr->platform);
        ptr->platform = NULL;
      }
    free (ptr);
}
...
yajl_gen_status
gen_image_spec_schema_manifest_list_manifests_element (yajl_gen g, const image_spec_schema_manifest_list_manifests_element *ptr, const struct parser_context *ctx, parser_error *err)
{
    yajl_gen_status stat = yajl_gen_status_ok;
    *err = NULL;
    (void) ptr;  /* Silence compiler warning.  */
    ...
    if ((ctx->options & OPT_GEN_KEY_VALUE) || (ptr != NULL && ptr->platform != NULL))
      {
        ...
        if (stat != yajl_gen_status_ok)
            GEN_SET_ERROR_AND_RETURN (stat, err);
        stat = gen_image_spec_schema_manifest_list_manifests_platform (g, ptr != NULL ? ptr->platform : NULL, ctx, err);
        if (stat != yajl_gen_status_ok)
            GEN_SET_ERROR_AND_RETURN (stat, err);
      }
    stat = yajl_gen_map_close ((yajl_gen) g);
...
define_cleaner_function (image_spec_schema_manifest_list *, free_image_spec_schema_manifest_list)
image_spec_schema_manifest_list *
make_image_spec_schema_manifest_list (yajl_val tree, const struct parser_context *ctx, parser_error *err)
{
    __auto_cleanup(free_image_spec_schema_manifest_list) image_spec_schema_manifest_list *ret = NULL;
    *err = NULL;
    (void) ctx;  /* Silence compiler warning.  */
    if (tree == NULL)
      return NULL;
        ...
        if (tmp != NULL && YAJL_GET_ARRAY (tmp) != NULL)
          {
            ...
            for (i = 0; i < len; i++)
              {
                yajl_val val = values[i];
                ret->manifests[i] = make_image_spec_schema_manifest_list_manifests_element (val, ctx, err);
...
void
free_image_spec_schema_manifest_list (image_spec_schema_manifest_list *ptr)
{
    if (ptr == NULL)
        return;
    free (ptr->media_type);
    ...
    if (ptr->manifests != NULL)      {
        ...
        for (i = 0; i < ptr->manifests_len; i++)
          ...
          if (ptr->manifests[i] != NULL)
            {
              free_image_spec_schema_manifest_list_manifests_element (ptr->manifests[i]);
...
yajl_gen_status
gen_image_spec_schema_manifest_list (yajl_gen g, const image_spec_schema_manifest_list *ptr, const struct parser_context *ctx, parser_error *err)
{
    yajl_gen_status stat = yajl_gen_status_ok;
    *err = NULL;
    (void) ptr;  /* Silence compiler warning.  */
    ...
    if ((ctx->options & OPT_GEN_KEY_VALUE) || (ptr != NULL && ptr->manifests != NULL))
      {
        ...
        for (i = 0; i < len; i++)
          {
            stat = gen_image_spec_schema_manifest_list_manifests_element (g, ptr->manifests[i], ctx, err);
            if (stat != yajl_gen_status_ok)
                GEN_SET_ERROR_AND_RETURN (stat, err);
          }
        stat = yajl_gen_array_close ((yajl_gen) g);
...
image_spec_schema_manifest_list *
image_spec_schema_manifest_list_parse_file (const char *filename, const struct parser_context *ctx, parser_error *err)
{
image_spec_schema_manifest_list *ptr = NULL;size_t filesize;
    ...
    if (content == NULL)
      {
        if (asprintf (err, "cannot read the file: %s", filename) < 0)
            *err = strdup ("error allocating memory");
        return NULL;
      }ptr = image_spec_schema_manifest_list_parse_data (content, ctx, err);return ptr;
}
image_spec_schema_manifest_list * 
image_spec_schema_manifest_list_parse_file_stream (FILE *stream, const struct parser_context *ctx, parser_error *err)
{image_spec_schema_manifest_list *ptr = NULL;
size_t filesize;
    __auto_free char *content = NULL;
...
define_cleaner_function (yajl_val, yajl_tree_free)
 ...
 { 
  image_spec_schema_manifest_list *ptr = NULL;__auto_cleanup(yajl_tree_free) yajl_val tree = NULL;
    char errbuf[1024];
...
ptr = make_image_spec_schema_manifest_list (tree, ctx, err);return ptr; 
}
...
define_cleaner_function (yajl_gen, cleanup_yajl_gen)
 ...
 char * 
image_spec_schema_manifest_list_generate_json (const image_spec_schema_manifest_list *ptr, const struct parser_context *ctx, parser_error *err){
...
if (yajl_gen_status_ok != gen_image_spec_schema_manifest_list (g, ptr, ctx, err))  {
        if (*err == NULL)
