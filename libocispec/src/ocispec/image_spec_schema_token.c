...
#include "image_spec_schema_token.h"

#define YAJL_GET_ARRAY_NO_CHECK(v) (&(v)->u.array)
#define YAJL_GET_OBJECT_NO_CHECK(v) (&(v)->u.object)
define_cleaner_function (image_spec_schema_token *, free_image_spec_schema_token)
image_spec_schema_token *
make_image_spec_schema_token (yajl_val tree, const struct parser_context *ctx, parser_error *err)
{
    __auto_cleanup(free_image_spec_schema_token) image_spec_schema_token *ret = NULL;
    *err = NULL;
...
void
free_image_spec_schema_token (image_spec_schema_token *ptr)
{
    if (ptr == NULL)
        return;
    free (ptr->token);
...
yajl_gen_status
gen_image_spec_schema_token (yajl_gen g, const image_spec_schema_token *ptr, const struct parser_context *ctx, parser_error *err)
{
...
image_spec_schema_token *
image_spec_schema_token_parse_file (const char *filename, const struct parser_context *ctx, parser_error *err)
{
image_spec_schema_token *ptr = NULL;size_t filesize;
    ...
    if (content == NULL)
      {
        if (asprintf (err, "cannot read the file: %s", filename) < 0)
            *err = strdup ("error allocating memory");
        return NULL;
      }ptr = image_spec_schema_token_parse_data (content, ctx, err);return ptr;
}
image_spec_schema_token * 
image_spec_schema_token_parse_file_stream (FILE *stream, const struct parser_context *ctx, parser_error *err)
{image_spec_schema_token *ptr = NULL;
size_t filesize;
    __auto_free char *content = NULL;
...
define_cleaner_function (yajl_val, yajl_tree_free)
 ...
 { 
  image_spec_schema_token *ptr = NULL;__auto_cleanup(yajl_tree_free) yajl_val tree = NULL;
    char errbuf[1024];
...
ptr = make_image_spec_schema_token (tree, ctx, err);return ptr; 
}
...
define_cleaner_function (yajl_gen, cleanup_yajl_gen)
 ...
 char * 
image_spec_schema_token_generate_json (const image_spec_schema_token *ptr, const struct parser_context *ctx, parser_error *err){
...
if (yajl_gen_status_ok != gen_image_spec_schema_token (g, ptr, ctx, err))  {
        if (*err == NULL)
