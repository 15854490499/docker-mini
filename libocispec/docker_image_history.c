...
define_cleaner_function (docker_image_history *, free_docker_image_history)
docker_image_history *
make_docker_image_history (yajl_val tree, const struct parser_context *ctx, parser_error *err)
{
    __auto_cleanup(free_docker_image_history) docker_image_history *ret = NULL;
    *err = NULL;
    ...
    if (tree->type == yajl_t_object)
      {
        ...
        for (i = 0; i < tree->u.object.len; i++)
          {if (strcmp (tree->u.object.keys[i], "created")
                ...
                && strcmp (tree->u.object.keys[i], "created_by")
                && strcmp (tree->u.object.keys[i], "comment")
                && strcmp (tree->u.object.keys[i], "empty_layer")){
                if (ctx->options & OPT_PARSE_FULLKEY)
                  {
...
void
free_docker_image_history (docker_image_history *ptr)
{
    if (ptr == NULL)
        return;
    free (ptr->created);
...
yajl_gen_status
gen_docker_image_history (yajl_gen g, const docker_image_history *ptr, const struct parser_context *ctx, parser_error *err)
{
...
docker_image_history *
docker_image_history_parse_file (const char *filename, const struct parser_context *ctx, parser_error *err)
{
docker_image_history *ptr = NULL;size_t filesize;
    ...
    if (content == NULL)
      {
        ...
        return NULL;
      }ptr = docker_image_history_parse_data (content, ctx, err);return ptr;
}
docker_image_history * 
docker_image_history_parse_file_stream (FILE *stream, const struct parser_context *ctx, parser_error *err)
{docker_image_history *ptr = NULL;
size_t filesize;
    __auto_free char *content = NULL;
...
define_cleaner_function (yajl_val, yajl_tree_free)
 ...
 { 
  docker_image_history *ptr = NULL;__auto_cleanup(yajl_tree_free) yajl_val tree = NULL;
    char errbuf[1024];
...
ptr = make_docker_image_history (tree, ctx, err);return ptr; 
}
...
define_cleaner_function (yajl_gen, cleanup_yajl_gen)
 ...
 char * 
docker_image_history_generate_json (const docker_image_history *ptr, const struct parser_context *ctx, parser_error *err){
...
if (yajl_gen_status_ok != gen_docker_image_history (g, ptr, ctx, err))  {
        if (*err == NULL)
